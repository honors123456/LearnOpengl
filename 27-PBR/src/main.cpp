...
int main() {
  ...
  Shader pbrShader(SHADER_DIR "/objectShader.vert",
  ...
  // 背景面使用同一个 PBR 着色器，让米褐色背景也接受柔和光照。
  ...
  const std::array<glm::vec3, 5> spherePositions = {
      glm::vec3(-3.2f, 0.15f, 0.0f), glm::vec3(-1.6f, 0.15f, 0.0f),
      glm::vec3(0.0f, 0.15f, 0.0f), glm::vec3(1.6f, 0.15f, 0.0f),
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glm::mat4 view = glm::lookAt(camera.Position, glm::vec3(0.0f),
    ...
    glBindVertexArray(panelVAO);
    glm::mat4 panelModel =
        glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -2.2f));

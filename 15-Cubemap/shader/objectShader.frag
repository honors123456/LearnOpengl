...
void main()
{
    ...
    vec3 V = normalize(cameraPos - fragPos);      //从片段指向相机的观察方向
    vec3 I = normalize(fragPos - cameraPos);      //从相机射向片段的入射方向
    vec3 reflectionDir = reflect(I, N);           //环境镜像反射方向
    ...
    FragColor = vec4(texture(skybox, reflectionDir).rgb, 1.0);
}

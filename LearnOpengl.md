...
#### 推荐一：从零手写一个软件光栅化器 (Tiny Renderer)

**谁写的：** 

俄罗斯大牛 Dmitry V. Sokolov（这是图形学界赫赫有名的开源神作）。

**干什么的：** 

完全不使用任何显卡硬件加速（不调用 OpenGL，只用 C++ 和画像素的接口），从零亲手写一个 3D 渲染器！

##### **你将亲手实现：**

自己写代码画线（Bresenham 算法）、自己写代码对三角形进行光栅化填色、自己写透视矫正插值、自己写矩阵变换、自己写 Z-Buffer（深度测试），最后硬生生在屏幕上画出一个带纹理、带法线贴图的 3D 游戏角色。

##### **为什么能治愈虚无感**：

当你在控制台里不调用任何显卡 API，却亲手用纯 C++ 算出了一个会转动的 3D 玛雅头像时，你会彻底看穿 GPU 到底在底层对你隐瞒了什么。所有的矩阵、插值、齐次坐标会瞬间变得无比具象。

##### 代码量： 

整个项目核心代码通常只有 500 ~ 1000 行纯 C++。



#### 推荐二：手写微型物理渲染/延迟渲染管线 (Mini Deferred Renderer)

##### 干什么的：

把我们前面学过的 延迟渲染 (G-Buffer)、阴影映射 (Shadow Mapping) 和 PBR 物理材质 揉在一起，写一个 300 行左右的纯现代 OpenGL 微型架构。

##### 你将亲手实现：

开辟一个带 3 个附件的 MRT 帧缓冲（G-Buffer）。
往里面扔 50 个不同位置的箱子。
用全屏 Quad 把它合成为带阴影和 PBR 质感的最终画面。

##### 为什么能治愈虚无感：

它直接把你刚刚学过的“企业级架构”落地成一个可执行的 .exe。你可以亲手去删掉某一个 attachment，看看画面是不是立刻黑掉；或者故意把光照循环的衰减系数改大，看着场景瞬间爆炸。



#### 推荐三：开源游戏引擎项目——**Hazel Engine**

##### 作者：**TheCherno（Yan Chernikov）** 

##### 介绍：

##### 绝对的“工业级 C++ 架构设计”：

TheCherno 曾经是 EA/Dice 的游戏引擎工程师。他的代码风格极其现代、优雅、面向对象且高度模块化。在 Hazel 里，你看不到那种写在同一个文件里的面条代码，而是有严密的：

核心层（Core）、事件系统（Event System）
渲染抽象层（Renderer API：支持 OpenGL，未来计划 Vulkan）
场景与实体组件系统（ECS 思想）
依托 Dear ImGui 的极其惊艳的引擎编辑器界面（Hazelnut Editor）。



#### 推荐四：光线追踪周末系列 (Ray Tracing in One Weekend)

##### 是谁： 

经典开源迷你电子书（Peter Shirley 著）。

##### 干什么的：

彻底抛弃光栅化（Rasterization），用经典的**光线追踪（Ray Tracing）**理念，从零在周末两天里写出一个能渲染出晶莹剔透玻璃球、金属反射、以及完美阴影的 3D 渲染器。

##### 你将亲手实现：

射线与球体的求交数学计算、漫反射随机散射、菲涅尔折射反射。最后输出一张极其惊艳的 .ppm 格式的物理光追大片。

##### 为什么能治愈虚无感：

代码量极其短小（一个周末就能肝完），但视觉震撼力极大。你会发现原来大片里的物理光影，只需要几百行递归数学代码就能搞定。



#### 推荐五：手写 2D 游戏引擎：Breakout (LearnOpenGL 官方实战)

##### 干什么的：

LearnOpenGL 官方教程自带的压轴实战。用纯 C++ 和 OpenGL 从零写一个完整的《打砖块》游戏。

##### 你将亲手实现：

精灵渲染系统（Sprite Renderer）、AABB 碰撞检测算法、粒子系统（就是我们前面学的那个动态实例化雪花特效）、音效和关卡管理。

##### 为什么能治愈虚无感：

前面那些都是技术组件，而这个项目是一个真正的、可以拿来玩的游戏。当你亲手操纵挡板把球弹飞、砸碎砖块并触发粒子爆炸时，那种“我不仅懂底层，我还能造产品”的成就感是无与伦比的。
...
## 项目一：Mini-Marmoset（PBR / IBL 材质与环境烘焙工作室）

### 阶段 1：几何体生成与直接光 PBR 闭环（耗时：2 天）

**核心任务清单**

- [ ] **工程初始化**：创建空工程，仅引入 `Glad`、`GLFW`（或 Qt 的 `QOpenGLWidget`）和 `GLM`，配置最基础的视口与清屏背景色。
- [ ] **参数化生成球体网格（UV Sphere）**：
  - 用两层 `for` 循环（按经纬度分割，如 $64 \times 64$ 分段）在 CPU 侧计算顶点的 `Position`、`Normal`、`TexCoord`；
  - 计算三角形索引数组（EBO/IBO），灌入 VBO/VAO。
- [ ] **编写基础 PBR 着色器（Direct PBR Shader）**：
  - 实现 Cook-Torrance BRDF 核心函数：NDF（GGX）、Fresnel（Schlick）、Geometry（Smith-GGX）；
  - 在 Uniform 中预设 4 个点光源（位置、颜色）；
  - 材质参数（$Albedo$、$Roughness$、$Metallic$）先硬编码在 Shader 顶端或通过 Uniform 传入；
  - 片段着色器末尾加入 Tone Mapping（Reinhard）与 Gamma 2.2 校正。

> **阶段验收标准**：屏幕中心出现一个光照正确的三维球体，高光随视角移动平滑过渡，无自发光死白或阴影边缘撕裂。用 RenderDoc 抓帧，确认只有 1 个 Draw Call，且顶点属性绑定无泄漏。

### 阶段 2：交互面板与 $5 \times 5$ 材质矩阵（耗时：2 天）

**核心任务清单**

- [ ] **接入 UI 控件**：
  - 集成 ImGui 或直接使用 Qt 的侧边栏滑动条（`QSlider`、`QColorDialog`）；
  - 绑定浮点变量：`roughness`（0.05 ~ 1.0）、`metallic`（0.0 ~ 1.0）、`albedo`（RGB）。
- [ ] **绘制 $5 \times 5$ 材质对比球体阵列**：
  - 双重循环渲染 25 个球体，每个球体通过 `Model` 矩阵平移到不同坐标；
  - 横轴逐列递增 `roughness`（0.05 到 1.0），纵轴逐行递增 `metallic`（0.0 到 1.0）；
  - 观察从非金属塑料质感（微弱白高光）到全金属质感（带自身基色高光）的视觉变化。
- [ ] **集成纹理贴图驱动**：
  - 引入 `stb_image`，支持从磁盘载入材质贴图（Albedo, Normal, Roughness, Metallic, AO）；
  - 着色器支持宏开关或条件分支：无贴图时使用纯数值，有贴图时采样纹理驱动材质。

> **阶段验收标准**：通过 UI 滑动条调节参数时，单个测试球的粗糙度与金属度实时响应；材质阵列中每一颗球呈现出清晰的材质梯度，粗糙金属表面反射漫散，光滑金属表面高光紧凑锐利。



### 阶段 3：离线 IBL 烘焙自动化管线（耗时：3~4 天）

**核心任务清单**

- [ ] **环境全景图转立方体贴图（2D $\to$ Cubemap）**：
  - 使用 `stb_image.h` 的 `stbi_loadf` 读取 32 位浮点 `.hdr` 贴图；
  - 建立 FBO 挂载 6 面的 Cubemap 纹理（$512 \times 512$ 分辨率，格式 `GL_RGB16F`）；
  - 设置 90 度 FOV 摄像机，面向 6 个方向渲染单位立方体，完成等距柱状投影（Equirectangular）采样转换。
- [ ] **漫反射辐照度卷积（Irradiance Map）**：
  - 创建 $32 \times 32$ 分辨率的 Cubemap；
  - 编写半球黎曼和卷积着色器，两层循环采样周围天空光，烘焙出低频环境漫反射贴图。
- [ ] **镜面反射预过滤贴图（Prefiltered Map）**：
  - 创建 $128 \times 128$ 分辨率并开启 Mipmap（5 层 Mip）的 Cubemap；
  - 编写蒙特卡洛重要性采样 Shader，针对每一层 Mip 传入对应的 `roughness` 进行离线模糊，逐层写入 FBO。
- [ ] **积分 2D BRDF LUT**：
  - 创建 $512 \times 512$ 的 2D 贴图（格式 `GL_RG16F`）；
  - 渲染全屏四边形，计算 Schlick-GGX 几何项与菲涅尔项的双重积分，将结果保存在贴图的 R 和 G 通道。

> **阶段验收标准**：在磁盘或内存中生成 4 张预计算资产。用 RenderDoc 查看纹理附件，Irradiance Map 呈现为极致平滑的模糊全景；Prefiltered Map 从 Mip 0 到 Mip 4 清晰度逐级衰减；BRDF LUT 呈现红绿渐变特征图。



### 阶段 4：IBL 全局光照汇总与工作台封装（耗时：2 天）

**核心任务清单**

- [ ] **PBR 着色器环境光合并**：
  - 将 `irradianceMap`、`prefilterMap`、`brdfLUT` 绑定到固定纹理槽位；
  - 片段着色器实现 Split-Sum 近似公式，合并漫反射环境光与镜面反射环境光；
  - 引入 `ao`（环境遮挡贴图）调制总 Ambient 输出。
- [ ] **绘制天空盒背景**：
  - 编写 Skybox 着色器，使用 `envCubemap` 渲染背景；
  - 深度测试设置为 `GL_LEQUAL`，并将天空盒顶点 $Z$ 坐标强制重定向到远平面（`gl_Position = pos.xyww`）。
- [ ] **环境资产热切换器**：
  - UI 增加“加载 HDR 环境”按钮，支持拖入新的 `.hdr` 图像，一键清空并重跑烘焙管线。

> **阶段验收标准**：场景中的金属球表面不仅能反弹点光源，还能清晰映照出天空盒的高动态反射；粗糙度拉高时，环境倒影自动过渡为柔和的磨砂光晕。





## 项目二：Sponza 延迟渲染与多光源后处理管线

### 阶段 1：场景导入与基础前向管线验证（耗时：2 天）

**核心任务清单**

- [ ] **模型与材质解析**：
  - 引入 `Assimp` 库，编写 `Model` 和 `Mesh` 类；
  - 载入经典的 `Crytek Sponza`（或简化版室内建筑模型），解析多子网格的 Diffuse、Specular、Normal 贴图。
- [ ] **基础视口控制器**：
  - 实现 FPS 风格相机（WASD 移动、鼠标控制 Pitch/Yaw），支持可调平移速度与无级变焦。
- [ ] **前向光照基线验证**：
  - 编写一个基础 Forward Shader，仅带一个方向光，验证场景所有网格法线朝向、UV 映射及深度测试完全正确。

> **阶段验收标准**：能在 Sponza 宫殿内自由穿梭，视角流畅（60fps 以上），各柱体与拱门无几何错位或面剔除错误。

### 阶段 2：G-Buffer 架构与多目标离屏渲染（MRT）（耗时：2 天）

**核心任务清单**

- [ ] **配置 G-Buffer 帧缓冲**：
  - 创建一个 `gBuffer` FBO，并挂载 3 张颜色纹理与 1 个深度缓冲对象（RBO/Texture）：
    - `gPosition`：`GL_RGBA16F`（RGB 存世界/视图坐标，A 通道备用）；
    - `gNormal`：`GL_RGBA16F`（RGB 存法线向量）；
    - `gAlbedoSpec`：`GL_RGBA8`（RGB 存漫反射颜色，A 通道存高光强度）。
  - 调用 `glDrawBuffers(3, attachments)` 开启多渲染目标输出。
- [ ] **编写几何阶段着色器（Geometry Pass）**：
  - 在片段着色器中同时向 `layout (location = 0/1/2)` 输出位置、法线与反照率数据；
  - 结合法线贴图进行 TBN 空间变换，将切线空间法线转到世界空间写入 `gNormal`。
- [ ] **多视口屏幕调试器（G-Buffer Visualizer）**：
  - 编写一个全屏矩形着色器，按数字键 1/2/3/4 在屏幕上切换查看：原始深度图（线性化显示）、法线图（映射至 $[0, 1]$ 范围）、反照率图。

> **阶段验收标准**：按下切换键时，法线图呈现红绿蓝三维朝向特征，位置图随视角推移呈现渐变色；RenderDoc 中能一次捕获到完整填充的 3 张 G-Buffer 纹理。

### 阶段 3：百级动态光源光照阶段（Lighting Pass）（耗时：2 天）

**核心任务清单**

- [ ] **生成动态光源系统**：
  - 创建一个数据结构存储 100~200 个动态点光源（位置、颜色、衰减半径）；
  - 在 CPU 逻辑更新中让光源在 Sponza 大厅内做正弦轨迹浮动。
- [ ] **编写延迟光照片段着色器**：
  - 渲染一个覆盖屏幕的全屏四边形（Screen Quad），关闭深度写入（`glDepthMask(GL_FALSE)`）；
  - 采样 G-Buffer 的 3 张纹理恢复该像素的坐标、法线和颜色；
  - 循环遍历所有光源，计算 Blinn-Phong 或 Cook-Torrance 漫反射与高光，加入距离平方反比衰减；
  - 最终写入默认帧缓冲（Back Buffer）。
- [ ] **光源实体可视化**：
  - 复制 G-Buffer 的深度缓冲到默认缓冲（`glBlitFramebuffer`）；
  - 开启前向着色绘制几十个发光的小立方体或小球，用来直观指示动态光源所在的位置。

> **阶段验收标准**：大厅内数百个彩光小球四处飘动，地面和柱子实时投射出相互交织的彩色光晕；在 100 个光源下，帧率相比前向渲染无断崖式下跌。

### 阶段 4：SSAO 屏幕空间环境光遮蔽集成（耗时：3 天）

**核心任务清单**

- [ ] **构建采样核心与噪声纹理**：
  - 在 CPU 端生成 64 个半球空间随机采样向量（向法线方向偏置，更靠近中心）；
  - 生成 $4 \times 4$ 个随机旋转向量，填充为平铺的噪点纹理（Noise Texture）。
- [ ] **编写 SSAO 着色器**：
  - 读取 G-Buffer 的位置和法线图；
  - 利用平铺噪点扰动采样方向，在视图空间发射 64 根采样探针；
  - 将探针投影回屏幕 UV，采样实际深度图进行遮挡对比（带 Bias）；
  - 引入 `rangeCheck` 平滑去除几何边缘暗斑。
- [ ] **SSAO 双边模糊去噪（Bilateral / Box Blur）**：
  - 创建中间 FBO，用 $4 \times 4$ 均值模糊核平滑噪点，同时保留物体的硬边缘。
- [ ] **最终合成**：
  - 将模糊后的 `ssaoTexture` 作为环境光乘数乘入 Lighting Pass。

> **阶段验收标准**：宫殿拱门内侧凹角、柱子底部与地面接缝处出现自然的接触阴影；关闭/开启 SSAO 时画面纵深感对比极为显著。



## 项目三：工业级 3D 几何检视与交互测量仪

### 阶段 1：工业视口与无限参考网格（耗时：2 天）

**核心任务清单**

- [ ] **轨道相机（Arcball Orbit Camera）实现**：
  - 实现工业 CAD 软件通用的观察逻辑：
    - 鼠标中键按住平移（Pan）；
    - 鼠标滚轮围绕焦点无级缩放（Zoom）；
    - 鼠标左键/右键围绕三维焦点旋转（Rotate），消除万向节死锁（Gimbal Lock）。
- [ ] **无限渐变地面网格（Infinite Grid Shader）**：
  - 不使用大尺寸网格 Mesh，仅渲染一个覆盖视口的四边形；
  - 在着色器中通过反投影矩阵将近/远平面点投射到世界空间的 $Y=0$ 平面上；
  - 利用导数函数（`fwidth`）绘制带有抗锯齿的主辅网格线（例如 1m 主网格，0.1m 辅网格），远端按视线距离淡出。
- [ ] **视口坐标轴指示器（Gizmo）**：
  - 在屏幕右上角或左下角划分一个独立小 Viewport，绘制随主相机旋转的 RGB 三轴坐标指示器（X-红，Y-绿，Z-蓝）。

> **阶段验收标准**：网格线在视角旋转时平滑延伸且无摩尔纹锯齿；相机缩放与旋转严格以观察中心为锚点，无漂移失控感。

### 阶段 2：屏幕射线投射与点/面求交拾取（耗时：2 天）

**核心任务清单**

- [ ] **2D 屏幕坐标反投影生成 3D 射线**：
  - 获取鼠标点击的窗口坐标 $(x, y)$，归一化至 NDC 坐标 $[-1, 1]$；
  - 分别与 `ProjectionMatrix * ViewMatrix` 的逆矩阵相乘，计算世界空间中的射线起点 $\mathbf{O}$ 与单位方向向量 $\mathbf{D}$。
- [ ] **射线与三角网格求交算法（Ray-Mesh Intersection）**：
  - 在 CPU 端实现 **Möller–Trumbore** 快速求交算法；
  - 遍历模型的所有三角形面片，计算射线的相交距离 $t$ 及重心坐标 $(u, v)$；
  - 筛选出距离最近的有效相交点世界坐标 $\mathbf{P} = \mathbf{O} + t\mathbf{D}$。
- [ ] **拾取指示器绘制**：
  - 鼠标移动时在相交点处实时渲染一个微型黄色高亮小球，并在表面绘制被拾取三角形的线框边缘。

> **阶段验收标准**：鼠标在模型表面划过时，黄色标记点严丝合缝地贴合网格表面移动；点击空白处时光标自动隐藏，求交计算耗时单次小于 2ms。

### 阶段 3：交互式空间测距与 HUD 标注系统（耗时：2 天）

**核心任务清单**

- [ ] **测量交互状态机**：
  - 设计状态转移逻辑：`IDLE`（等待第一次点击） $\to$ `SELECTING_FIRST` $\to$ `SELECTING_SECOND` $\to$ `FINISHED`。
- [ ] **空间动态虚线绘制**：
  - 选定起点后，鼠标移动时用动态 VBO（`glBufferSubData`）更新端点坐标；
  - 编写虚线着色器（通过片段在屏幕空间的距离或顶点累计长度，利用 `fract` 或 `sin` 舍弃片段），绘制一条连接两点的动态虚线。
- [ ] **投影分量计算与标线**：
  - 计算两点间的绝对欧氏距离 $L$，以及空间各分量：$\Delta X = \vert{}x_2 - x_1\vert{}$、$\Delta Y = \vert{}y_2 - y_1\vert{}$、$\Delta Z = \vert{}z_2 - z_1\vert{}$；
  - 可选绘制三轴正交辅助投影线。
- [ ] **三维空间文字/数值标注（Billboard / Screen Overlay）**：
  - 将两点中心的世界坐标通过当前矩阵前向投影回屏幕 2D 坐标；
  - 在该屏幕坐标上使用 Qt 的 `QPainter`、ImGui 浮动文本或 FreeType 纹理，悬浮打印测距数值（如 `Dist: 1.452 m`）。

> **阶段验收标准**：在模型上点两下，两点之间拉出一条带有长度标注的测量线；转动相机时，标线两端牢牢钉在物体表面，文字数值始终面向屏幕且无遮挡翻转。

### 阶段 4：大体量点云流式渲染与伪彩色映射（耗时：2 天）

**核心任务清单**

- [ ] **点云数据流式加载（PLY / XYZ 格式）**：
  - 编写单文件解析器，支持读取 50 万 ~ 200 万级纯点云数据（$X, Y, Z$ 及可选强度/反射率）；
  - 将点数据直接装配为单一紧凑的 VBO，开启 `glEnable(GL_PROGRAM_POINT_SIZE)`。
- [ ] **点云伪彩色热力图着色器**：
  - 顶点着色器根据点的高度（$Z$ 轴坐标）或反射强度进行归一化；
  - 片段着色器实现彩虹渐变映射算法（从蓝 $\to$ 青 $\to$ 绿 $\to$ 黄 $\to$ 红），直观反映空间高度分层。
- [ ] **几何模型与点云的混合遮挡管理**：
  - 严格管理深度测试（`glDepthFunc(GL_LESS)`）；
  - 保证实心 CAD 网格与外部离散点云共同绘制时，空间前后遮挡关系分明，不出现深度写入冲突导致的闪烁。

> **阶段验收标准**：顺畅加载百万级点云并达到 60fps 以上；点云按高度清晰呈现热力色带；测距系统能够直接拾取点云中的具体散点并完成空间测距。

### 执行建议：如何推进这 3 张清单？

1. **不要并发推进**：先选定一个项目（首选**项目一**打透 PBR/IBL，或选**项目二**吃透渲染管线）；
2. **每一步以“代码敲完、在屏幕上肉眼看到该阶段特征”为完成标志**，不要提前写下一个阶段的代码；
3. **遇到黑屏或状态异常，直接上 RenderDoc 抓帧查看 Buffer 绑定位**，严禁对着代码死猜。
